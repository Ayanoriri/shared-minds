...
const state = { nodes: [], edges: [], running:true, autoEvents:true, lastAutoTick:0, autoInterval:1800 };
...
function autoEvent(){
  // pick one or more random nodes to act on concurrently so multiple bubbles are active
  if(state.nodes.length === 0) return;
  // sometimes trigger several nodes at once (2-4)
  const multiChance = 0.28;
  const triggers = (Math.random() < multiChance) ? (2 + Math.floor(Math.random() * 3)) : 1;

  const doActionOnIndex = (idx) => {
    const n = state.nodes[idx];
    if(!n) return;
    // rarely skip the user's bubble to allow it to participate too
    if(localUser && n.userId === localUser.uid && Math.random() > 0.9) return;
    ...
    if(mode < 0.16){
      ...
      // hover-like
      n.attention = Math.min(1, n.attention + 0.18 + Math.random() * 0.22);
