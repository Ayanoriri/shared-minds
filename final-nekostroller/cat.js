...
function drawBlinkLines() {
  ...
  if (window.blinkState === 'closed') {
    ...
    const eyeColor = (window && window.PALETTE && window.PALETTE.green) ? window.PALETTE.green : '#838A2D';
...
function _maybeWireColorInput() {
  ...
  if (reset) {
    reset.addEventListener('click', function() {
      ...
      // Reset the match display text
      if (typeof window.resetMatchDisplay === 'function') {
        window.resetMatchDisplay();
      }
    });
  }
